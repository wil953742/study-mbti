  ...
  mounted(): void {
    ...
    switch (mainPath) {
      ...
      case 'type':
        ...
        if (MBTI.includes(subPath.toUpperCase())) {
          mbtiType = subPath.toUpperCase();
          path += `/${subPath.toUpperCase()}`;
        }

        if (!popState)
          history.pushState({ mainPath, mbtiType }, 'typepage', path);
        new Type($main, { value: mbtiType });
        break;
      default:
